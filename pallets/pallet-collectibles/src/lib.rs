...
pub mod pallet {
	...
	pub enum Event<T: Config> {
		...
		/// A collectible was succesfully transferred.
		TransferSucceeded { from: T::AccountId, to: T::AccountId, collectible: [u8; 16] },
		/// The price of collectible was succesfully set.
		PriceSet { collectible: [u8; 16], price: Option<BalanceOf<T>> },
		/// The collectible was succesfully sold
		Sold {
			seller: T::AccountId,
			buyer: T::AccountId,
			collectible: [u8; 16],
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn gen_unique_id() -> ([u8; 16], Color) {
			// Create randomness
		...
		pub fn do_transfer(collectible_id: [u8; 16], to: T::AccountId) -> DispatchResult {
			...
			// Ensure collectible doesn't transfer to self
